...
class VBFBDT {
  public:
    ...
    VBFBDT(const std::string& weightsFilePath) {
        ...
        m_reader->AddVariable("delta_yjj",&m_bdt_drap);
        //m_reader->AddVariable("pt_bal",&m_bdt_ptbal);
        m_reader->AddVariable("centrality",&m_bdt_zcen);
        //m_reader->AddVariable("omega",&m_bdt_omega);
        //m_reader->AddVariable("transverseMassLep",&m_bdt_transmasslep);
        ...
        m_reader->BookMVA("VBFBDT", weightsFilePath.c_str());
    }
    ...
    VBFBDT& operator=(const VBFBDT& other) {
        if (this != &other) {
            ...
            //m_reader->AddVariable("transverseRecoMassVariable",&m_bdt_transmasslep); // For transverse-reco mass ratio
            m_reader->AddSpectator("eventNumber", &m_bdt_eventNumber); // For deterministic split
            m_reader->BookMVA("VBFBDT",  other.m_weightsFilePath);
        }
        return *this;
    }

    void update(float mjj, float drap, float zcen, float eventNumber) { // float transmasslep, float ptbal, float omega, 
        m_bdt_mjj = mjj;
        m_bdt_drap = drap;
        //m_bdt_ptbal = ptbal;
        m_bdt_zcen = zcen;
    ...
    double evaluate() {
        double bdtScore = m_reader->EvaluateMVA("VBFBDT");
        reset();
        return bdtScore;
    }
